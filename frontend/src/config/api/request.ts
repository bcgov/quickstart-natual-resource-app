...
const getUrl = (config: APIConfig, options: ApiRequestOptions): string => {
  ...
  const path = options.url
    ...
    .replaceAll(/{(\w+)}/g, (substring: string, group: string) => {
...
export const sendRequest = async <T>(
  ...
  options.middleware?.forEach((middleware) => {
    ...
    if (middleware.response || middleware.failure)
      ids.res = axiosClient.interceptors.response.use(middleware.response, middleware.failure);
    interceptorIds.push(ids);
  });
  ...
  try {
    ...
    // Eject all interceptors that were added for this request
    interceptorIds.forEach(({ req, res }) => {
      if (typeof req === 'number') axiosClient.interceptors.request.eject(req);
      if (typeof res === 'number') axiosClient.interceptors.response.eject(res);
    });
  }
};
